import jsPDF from "jspdf";
import "jspdf-autotable";

// Date Fns is used to format the dates we receive
// from our API call
import { format } from "date-fns";

// define a generatePDF function that accepts a reports argument
const generatePDF = reports => {

  //initialize jsPDF
  const doc = new jsPDF();
//  var font = 'Rubik';
//  var callAddFont = function () {
//      this.addFileToVFS('Rubik-Regular.ttf', font);
//      this.addFont('Rubik-Regular.ttf', 'Rubik', 'normal');
//  };
//  jsPDF.API.events.push(['addFonts', callAddFont])
//  doc.addFont("Rubik")
//  doc.setFont("Rubik"); // set font




  // define the columns we want and their titles
  const tableColumn = [ "תיאור","שם עובד", "מספר פרויקט", "תאריך דיווח", "כניסה", "יציאה"];
  // define an empty array of rows
  const tableRows = [];

  // for each report pass all its data into an array
  reports.forEach(report => {
    const reportData = [
      report.worker_id,
      report.project_id,
      report.reporting_date,
      report.start_time,
      report.end_time,
      report.description,
      // called date-fns to format the date on the ticket
      format(new Date(report.reporting_date), "yyyy-MM-dd")
    ];
    // push each tickcet's info into a row
    tableRows.push(reportData);
  });


  // startY is basically margin-top
  doc.autoTable(tableColumn, tableRows, { startY: 20 });
  const date = Date().split(" ");
  // we use a date string to generate our filename.
  const dateStr = date[0] + date[1] + date[2] + date[3] + date[4];
  // ticket title. and margin-top + margin-left
  let text = "דוח חדש:"
  doc.text(text, 14, 15, {lang: 'he',align: 'right',font: 'Rubik', isSymmetricSwapping: true, isInputVisual: true, isOutputVisual: false});
  // we define the name of our PDF file.
  doc.save(`report_${dateStr}.pdf`);
};

export default generatePDF;